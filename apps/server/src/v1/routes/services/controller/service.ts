...
export default class ServiceController {
  ...
  static async getById(req: Request, res: Response) {
    ...
    try {
      ...
      if (!data) {
        return res
          .status(404)
          .json({ message: "Servicio no encontrado", data: null });
      }
      return res.json({ message: "OK", data });
    } catch (err: any) {
      return res
  ...
  static async create(req: Request, res: Response) {
    ...
    try {
      ...
      return res
        .status(201)
        .json({ message: "Servicio creado con éxito", data });
    } catch (err: any) {
  ...
  static async update(req: Request, res: Response) {
    ...
    try {
      ...
      return res.json({ message: "Servicio actualizado con éxito", data });
    } catch (err: any) {
      if (err.message?.includes("UNIQUE")) {
        return res.status(409).json({
          message: "Ya existe un servicio con ese nombre",
  ...
  static async remove(req: Request, res: Response) {
    ...
    try {
      ...
      return res.json({ message: "Servicio desactivado (soft-delete)", data });
    } catch (err: any) {
      return res
        .status(500)
        .json({ message: err.message ?? "Error interno", data: null });
